...
public class NSVoice extends AbstractPropertyDictionary<NSVoice.VoiceProperty> {
    ...
    public int getAge() {
        try {
            return getInt(VoiceProperty.Age); // cause NPE why? (because of deprecation?)
        } catch (NullPointerException e) {
            return -1; // TODO check specs.
        }
    }

    public VoiceGender getGender() {
        return getEnum(VoiceGender.class, VoiceProperty.Gender);
    }
    ...
    public NSArray getSupportedCharacters() {
        return getValueAsType(VoiceProperty.SupportedCharacters, NSArray.class);
    }

    public NSArray getIndividuallySpokenCharacters() {
        return getValueAsType(VoiceProperty.IndividuallySpokenCharacters, NSArray.class);
    }
