...
package org.jvoicexml.jsapi2.mac.rococoa;
...
public abstract class NSSpeechSynthesizer extends NSObject {
    ...
    private static final Logger logger = System.getLogger(NSSpeechSynthesizer.class.getName());
     ...
     *
     * @see NSSpeechSynthesizer#getProperty(SpeechProperty)
     * @see NSSpeechSynthesizer#setProperty(SpeechProperty, NSObject)
     */
    public enum SpeechProperty implements NativeEnum<NSString> {
        StatusProperty, ErrorsProperty, InputModeProperty, CharacterModeProperty,
        NumberModeProperty, RateProperty, PitchBaseProperty, PitchModProperty,
        VolumeProperty, SynthesizerInfoProperty, RecentSyncProperty, PhonemeSymbolsProperty,
     ...
     *
     * @objc.protocol NSSpeechSynthesizerDelegate
     * @see "https://developer.apple.com/documentation/appkit/nsspeechsynthesizerdelegate?language=objc"
     */
    ...
    public static NSSpeechSynthesizer synthesizerWithVoiceIdentifier(String voiceIdentifier) throws IllegalArgumentException {
        return CLASS.alloc().initWithVoice(voiceIdentifier != null ? NSString.stringWithString(voiceIdentifier) : null);
    }

    /**
     * Construct a new synthesizer that speaks with a specified voice.
     *
     * @param voice the voice to use, or null to use the default voice
     * @return the newly created synthesizer
...
logger.log(Level.INFO, "voice: " + (voice != null ? voice.getIdentifier() : "null"));
     ...
     */
    public synchronized void setDelegate(NSSpeechSynthesizerDelegate delegate) {
        if (delegate != null) {
...
logger.log(Level.TRACE, String.format("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ delegate: %16x", this.delegateProxy.id().longValue()));
    ...
    public static class NSSpeechStatus extends AbstractPropertyDictionary<NSSpeechStatus.StatusProperty> {
        public enum StatusProperty implements NativeEnum<NSString> {
            ...
            @Override public NSString getNativeValue() {
                return value;
            }
        }
    ...
    public boolean setVoice(NSVoice voice) {
        // CurrentVoiceProperty basically takes a map with 2 keys, creator and id like an old school VoiceSpec spec (see SpeechSynthesis.h)
    ...
    /** Describes how to set the delimiters used to embed commands in text to be spoken */
    ...
    public void reset() {
        setProperty(SpeechProperty.ResetProperty, null); // pass null for computer speakers
    }
    ...
    // Those methods that will simply be called directly are public,
    // the rest are 'package' - wrapper methods can be found above.
    ...
    abstract NSSpeechSynthesizer initWithVoice(NSString voiceIdentifier);
